...
def veg_list_category():
    ...
    flag = 'veg'
    cur=mysql.connection.cursor()
    sql ='Select name from veg'
    cur.execute(sql)
    mysql.connection.commit()
    myresult =cur.fetchall()
    for i in range(0,len(myresult)):
        veg_list += "Name: " + myresult[i]['name'] + ","
...
def veg_after_ordering(flag,user_input):
    ...
    cur=mysql.connection.cursor()
    sql ='INSERT INTO user_details(id,name,street,place,category,food,status,time,phone) VALUES("%s","%s","%s","%s","%s","%s","%s","%s","%s")'%(user_id,name,street,place,flag,user_input,status,current_time,phone)
    cur.execute(sql)
    mysql.connection.commit()
